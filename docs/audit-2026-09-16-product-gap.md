...
| 退出码契约（P0-5 延伸） | ✅ 完成 | 同一个形状栽了三次：`serve`（没起来返回 0）、`restore`（非交互静默不做事返回 0）、`skills install/search`（DNS 失败返回 0）。统一收敛到 `_ok_or_error()`；新增静态扫描禁止「`print(可失败调用)` + `return 0`」再现 |
| 备份/恢复真机往返 + 确认契约 | ✅ 完成 | 真跑 `backup → 破坏 → restore`：带 `--yes` 恢复 8 个文件、内容一字不差、配置回滚、自动留 pre-restore 快照。查出并修掉「非交互下静默不做事却返回 0」（改 rc=2 + 提示加 `--yes`；同样修了 `sessions prune`） |
| 网关真机端到端 + 鉴权留痕 | ✅ 完成 | 真起服务打真请求：无/错 token → **401**，对 token → **200**；健康页免鉴权。查出两处并修掉：`uiu serve` 没起来却返回 0；**401 拒绝不进审计**（现在写 `gateway_auth_denied` + 路径/来源）。3 条测试真起 HTTP server 验证 |
| npm 发布产物级端到端 | ✅ 完成 | 用 `npm pack` 的真实 tarball 解包 → 跑 postinstall → 启动器 `uiu version`/`--json` 全通（135s）。过程中查出 **`files` 白名单漏了 `lib/`** 会让 npm 用户 MODULE_NOT_FOUND 装不上；已修并加 4 条打包契约测试 |
