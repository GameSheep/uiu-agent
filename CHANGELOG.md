...
- 消息区**流式 markdown 渲染**（代码块/列表/表格高亮，80ms 节流），工具调用单行折叠、Enter 展开详情。
- **多行输入**（Shift+Enter 换行、Enter 发送，可配置）、slash 命令补全、历史搜索（Ctrl+R）。
- 快捷键体系（Ctrl+N 新会话 / Ctrl+S 侧栏 / Ctrl+E 命令面板 / Ctrl+L 清屏 / ? 帮助浮层 等）。
